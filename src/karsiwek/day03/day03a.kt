...
fun drawPipes(input: String, map: HashMap<Pair<Int, Int>, String>, symbol: String) {
    ...
    for (direction in directions) {
        ...
        for(i in 1..direction.second){
            ...
            if(map.containsKey(newPoint)){
                if(!map.get(newPoint)!!.equals(symbol)){
                    map.put(newPoint, "X");
                }
            } else {
                map.put(newPoint, symbol);
            }
        }
    }
}
...
fun getResult(input1: String, input2: String): Int{
    val map: HashMap<Pair<Int, Int>, String> = HashMap();

    drawPipes(input1, map, "A");
    drawPipes(input2, map, "B");

  //  drawResult(map);
    return map.entries.filter { it.value.equals("X") }.map { Math.abs(it.key.first) + Math.abs(it.key.second) }.sorted().first();
}

fun drawResult(map: java.util.HashMap<Pair<Int, Int>, String>) {
        for (j in 0..200) {
            for(i in 0..200) {
            if(map.containsKey(Pair(i, j))) {
                System.out.print(map.get(Pair(i, j)));
            } else {
                System.out.print(".");
            }
        }
